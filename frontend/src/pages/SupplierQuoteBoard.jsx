...
export default function SupplierQuoteBoard() {
  ...
  function openFreight(item) { const sources = (item.cost_quotes || []).filter((q) => q.market.is_live); setFreightItem(item.id); setFreightSource(sources.length === 1 ? sources[0].id : null); }
  return <div className="exchange-page"><div className="exchange-page-head"><div><h1>Sàn báo giá NCC</h1><p>Thấy giá tốt hơn? Bấm Báo giá hàng hoặc Báo cước ngay trên dòng yêu cầu.</p></div><div className="exchange-page-controls"><select aria-label="Loại sàn" value={legacy ? "purchase" : "inquiry"} onChange={(e) => setParams(e.target.value === "purchase" ? { tab:"purchase" } : {})}><option value="inquiry">Theo yêu cầu giá</option><option value="purchase">Đề nghị mua (cũ)</option></select><button className="secondary" onClick={() => { setError(""); load(); }}>Làm mới</button></div></div>{legacy ? <LegacySupplierQuoteBoard /> : <>
    {error && <p className="error" role="alert">{error}</p>}
    <div className="market-overview-toolbar"><input aria-label="Tìm yêu cầu" placeholder="Tìm mã yêu cầu, mặt hàng, khách hàng…" value={search} onChange={(e) => setSearch(e.target.value)} /><span>{visible.length} yêu cầu · {updated ? "Cập nhật " + updated.toLocaleTimeString("vi-VN") : "Đang tải…"}</span></div>
    <div className="market-overview-scroll"><table className="market-overview"><thead><tr><th>Yêu cầu / Khách hàng</th><th>Mặt hàng / Số lượng</th><th>Điểm nhận → Điểm giao</th><th>Giá hàng đang chào</th><th>Tổng hàng + cước</th><th>Báo giá của anh/chị</th></tr></thead><tbody>{visible.map((item) => {
      const quotes = (item.cost_quotes || []).filter((q) => q.market.is_live);
      const offers = quotes.flatMap(offersFor);
      const pickups = [...new Set(quotes.map((q) => [q.street_address,q.ward_name,q.district_name,q.province_name,q.country_name].filter(Boolean).join(", ")).filter(Boolean))];
      const approved = item.sourcing_plans?.find((p) => p.status === "approved");
      const pending = item.sourcing_plans?.filter((p) => p.status === "proposed").length || 0;
      return <tr key={item.id} className={String(item.id) === focus ? "focused" : ""}><td><b>{item.price_request_code}</b><small>{item.customer_name || "—"}</small><small>{approved ? "Đã chốt" : pending ? pending + " đề xuất chờ duyệt" : "Đang nhận báo giá"}</small></td><td><b>{item.product_name || item.item_name || "Chưa khai báo mặt hàng"}</b><small>{item.quantity ? Number(item.quantity).toLocaleString("vi-VN") + " " + (item.unit || "") : "Chưa khai báo số lượng"}</small></td><td className="market-overview-route"><span>{pickups.length > 1 ? pickups.length + " điểm nhận theo nguồn hàng" : pickups[0] || "Điểm nhận: theo nguồn báo"}</span><small>→ {item.price_request_delivery_address || "Chưa khai báo điểm giao"}</small></td><td className="market-overview-price"><b>{priceRange(quotes.map((q) => q.market.goods_total))}</b><small>{quotes.length} nguồn · {quotes.some((q) => q.tax_basis !== "included") ? "Có giá chưa rõ / chưa gồm thuế" : quotes.length ? "Đã gồm thuế" : "Mời báo giá đầu tiên"}</small></td><td className="market-overview-price"><b>{approved ? money(approved.snapshot.landed_total) : priceRange(offers.map((o) => o.total))}</b><small>Cước: {priceRange(offers.map((o) => o.shipping))}</small><small>{approved ? "Phương án đã chốt" : offers.some((o) => o.total !== null && o.issues.length) ? "Tạm tính · còn thiếu điều kiện" : "Theo từng nguồn và cước đi kèm"}</small></td><td><div className="market-overview-actions">{supply && !approved && <><Link className="market-quick-quote" to={"/cost-quotes?price_request_item=" + item.id + "&new=1"}>+ Báo giá hàng</Link><button className="secondary" disabled={!quotes.length} onClick={() => openFreight(item)}>+ Báo cước</button></>}<button className="link-btn" onClick={() => setExpanded(expanded === item.id ? null : item.id)}>{expanded === item.id ? "Ẩn chi tiết" : pending ? "Xem / Duyệt (" + pending + ")" : "Xem chi tiết"}</button></div></td></tr>;
    })}</tbody></table>{!visible.length && <div className="exchange-empty">{loading ? "Đang tải bảng giá…" : "Không có yêu cầu phù hợp."}</div>}</div>
    <p className="market-overview-note">Giá hiển thị là khoảng giá đang chào, chưa phải xếp hạng. Tổng được tính riêng cho từng nguồn hàng và cước của nguồn đó.</p>
